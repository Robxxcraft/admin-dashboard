<template>
    <div class="main bg-slate-100">
      <div class="py-8 px-12">
        <div class="text-xl text-gray-00 font-bold mb-2">Manage transactions</div>
        ...
        <div class="overflow-auto rounded shadow-sm hidden md:block">
          <table class="shadow shadow-gray-100 w-full">
              <thead>
                <tr v-if="transactions.length > 0" class="text-white bg-gray-800 text-sm text-left">
                  ...
                  <template v-if="transactions.length > 0">
                    <tr class="row" v-for="(transaction, index) in transactions" :key="index">
                      <template v-for="col in columns">
                        <template v-if="Array.isArray(transaction[col])">
                          <td class="p-3 text-sm text-gray-800 font-bold">
                            <template v-for="nestItem in transaction[col]">
                              <div class="mb-0.5">{{'- '+nestItem.title }}</div>
                            </template>
                          </td>
                        </template>
                        <template v-else-if="col == 'total' || col == 'discountedTotal'">
                          <td class="p-3 text-sm text-gray-800 font-bold">{{ '$ '+currencyFormat(transaction[col]) }}</td>
                        </template>
                        <template v-else>
...
export default {
  ...
  setup() {
    ...
    onMounted(()=>{
      api.get('/carts?limit=10').then(res=>{
        transactions.value = res.data.carts
        columns.value = Object.keys(res.data.carts[0])
      })
    })
    ...
    return { transactions, columns, currencyFormat, search, searching, pagination }
  },
}
</script>