<template>
    <div class="relative h-screen p-6 md:p-12 z-10">
      ...
      <template v-if="country.name">
        <div class="mt-9 flex items-end">
          <div class="font-bold text-4xl me-3 text-gray-800">{{ country.name.common }}</div>
      ...
      <div class="mt-3 flex flex-wrap">
        <template v-if="country.altSpellings">
          <div v-for="(spell, index) in country.altSpellings" :key="index" class="bg-secondary text-white px-3 py-1 rounded-2xl mb-2 mr-2">{{spell}}</div>
        </template>
      </div>
      <div class="mt-6 flex flex-col md:flex-row md:space-x-5">
        <div class="w-full max-w-lg relative shadow-md border border-gray-50 p-6 mb-3 overflow-hidden">
          <div class="font-bold text-lg mb-1 text-gray-700">LatLong</div>
        ...
        <div class="w-full max-w-lg shadow-md flex flex-col space-y-1 p-6 mb-3">
          <template v-if="country.capital">
            <div>Capital: <span class="font-bold text-gray-800">{{country.capital[0]}}</span></div>
          </template>
          <div>Region: <span class="font-bold text-gray-800">{{country.region}}</span></div>
          <div>Subregion: <span class="font-bold text-gray-800">{{country.subregion}}</span></div>
        </div>
      </div>
      <div class="mt-6 flex flex-col md:flex-row md:space-x-5">
        <div class="w-full max-w-lg relative">
          <div class="font-bold text-lg text-gray-700">Calling Code</div>
          <div class="text-primary font-bold text-4xl my-2">{{callingCode}}</div>
          <div class="font-bold pb-3 text-gray-800"><span :onmouseover="hoverCountryWSCC" :onmouseleave="hoverCountryWSCC" class="cursor-pointer text-primary underline">{{countryWithSameCallingCode.length}} country</span> with this calling code</div>
          <template v-if="countryWithSameCallingCode">
            <div class="hover-card absolute shadow-lg rounded-lg p-3 text-white none" id="countryWSCC">
        ...
        <div class="w-full max-w-lg relative">
          <div class="font-bold text-lg text-gray-800">Currency</div>
          <div class="text-primary font-bold text-4xl my-2">{{currency}}</div>
          <div class="font-bold pb-3 text-gray-800"><span :onmouseover="hoverCountryWSC" :onmouseleave="hoverCountryWSC" class="cursor-pointer text-primary underline">{{countryWithSameCurrency.length}} country</span> with this currency</div>
          <template v-if="countryWithSameCurrency">
            <div class="hover-card absolute shadow-lg rounded-lg p-3 text-white none" id="countryWSC">
