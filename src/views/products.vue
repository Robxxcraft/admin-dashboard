<template>
  <div class="main bg-slate-100">
    <div class="py-8 px-12">
      <div class="text-xl text-gray-00 font-bold mb-2">Manage products</div>
      ...
      <div class="overflow-auto rounded shadow-sm hidden md:block">
        <table class="shadow shadow-gray-100 w-full">
            <thead>
              <tr v-if="products.length > 0" class="text-white bg-gray-800 text-sm text-left">
                <template v-for="i in columns">
                  <th :class="`p-3 font-semibold tracking-wide capitalize ${i == 'description' || i == 'images' ? 'w-80' : ''}`">{{i.replace(/([A-Z])/g, ' $1').trim()}}</th>
                </template>
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
              <template v-if="products.length > 0">
                <tr class="row" v-for="(transaction, index) in products" :key="index">
                  <template v-for="col in columns">
                    <template v-if="col == 'thumbnail'">
                      <td class="p-3 text-sm text-gray-800 font-bold">
                        <img :src="transaction[col]" alt="thumbnail">
                      </td>
                    </template>
                    <template v-else-if="col == 'images'">
                      <td class="p-3 text-sm whitespace-nowrap text-gray-800 font-bold">
                        <div class="flex flex-wrap">
                          <div v-for="image in transaction[col]">
                            <img :src="image" class="border" alt="thumbnail">
                          </div>
                        </div>
                      </td>
                    </template>
                    <template v-else>
                      <td class="p-3 text-sm text-gray-800 font-bold">{{ transaction[col] }}</td>
                    </template>
                  </template>
  ...
  setup() {
    ...
    onMounted(()=>{
      api.get('/products?limit=10').then(res=>{
        ...
        columns.value = Object.keys(res.data.products[0])
      })
    })
    ...
    return { products, columns, currencyFormat, search, searching, pagination }
  },
}
</script>